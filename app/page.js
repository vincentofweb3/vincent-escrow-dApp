...
// ERC-8183 UPDATE: Import the single Escrow contract details
import { VANTAGE_ESCROW_ADDRESS, VANTAGE_ESCROW_ABI } from "../lib/contract";
...
export default function Home() {
  ...
  ); // Standard dummy for native token
  ...
  // 2. Create the 'text' state variable
  ...
  // The exact integer ID from your viem/wagmi config
  const ARC_TESTNET_ID = 5042002;

  const handleSwitchNetwork = async () => {
    try {
      // Wagmi automatically handles adding the network if it's defined in your config
      await switchChainAsync({ chainId: ARC_TESTNET_ID });
    } catch (error) {
      console.error("Switch chain error caught:", error);

      // Fallback edge-case mechanism just in case Wagmi's injection fails
      if (window.ethereum) {
        try {
          await window.ethereum.request({
            ...
            params: [
              {
                // This dynamically converts the 5042002 integer to the correct hex (0x4cef52)
          ...
          console.error("Manual add failed:", addError);
        }
      }
    }
  };

  // const ARC_CHAIN_ID = "0x4cef52"; // Make sure this matches Arc's actual hex chain ID
  // const [isConnected, setIsConnected] = useState(false);
  ...
  //     // Fallback fallback edge-case mechanism
  //     if (window.ethereum) {
  //       try {
  //         await window.ethereum.request({
  //           method: "wallet_addEthereumChain",
  ...
  //     // 1. Check initial connection and chain on page load
  ...
  //     // 2. Set up event listeners for real-time changes
  ...
  // Derived state to determine if network is correct
  const isCorrectNetwork = chainId === ARC_TESTNET_ID;
  ...
  // ERC-8183 UPDATE: Track Job ID instead of contract address
  const [activeJobId, setActiveJobId] = useState(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("vantage_active_job");
  ...
  // --- CONTRACT READS (ERC-8183) ---

  // Read the single 'jobs' mapping based on the activeJobId
  const { data: jobData } = useReadContract({
    address: VANTAGE_ESCROW_ADDRESS,
    abi: VANTAGE_ESCROW_ABI,
    functionName: "jobs",
    args: activeJobId !== null ? [activeJobId] : undefined,
    query: { enabled: activeJobId !== null },
  });

  // Parse the returned job tuple into readable states for your UI
  // Tuple: [client, provider, evaluator, token, amount, state, deliverableHash]
  // JobState Enum: 0 = Open, 1 = Funded, 2 = Submitted, 3 = Terminal
  const jobState = jobData ? jobData[5] : null;
  const isFunded = jobState !== null && jobState >= 1;
  const isCompleted = jobState === 3;
  const isReleased = jobState === 3;
  const contractBalance = jobData ? jobData[4] : 0n;
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === "ESCROW_CREATED") {
          // If bot returns an ID now, save it. Otherwise log generic response.
          addLog(`Job Captured via Bot`, "SYSTEM");
        }
    ...
    const syncBot = async () => {
      try {
        ...
        if (
          ...
          setLastLoggedFile(data.fileName);
        }
      } catch (err) {
  ...
  // Persist Job ID to local storage
  if (typeof window !== "undefined" && activeJobId !== null) {
    localStorage.setItem("vantage_active_job", activeJobId.toString());
  }

  // --- DEPLOYMENT LOGIC (CREATE JOB) ---
  const handleCreateAndFund = async () => {
    ...
    try {
      ...
      const finalEvaluator = evaluatorAddr || address; // Use connected wallet as evaluator if empty

      // ERC-8183 UPDATE: Call createJob with 4 parameters on the single contract
      const txHash = await writeContractAsync({
        address: VANTAGE_ESCROW_ADDRESS,
        abi: VANTAGE_ESCROW_ABI,
        functionName: "createJob",
        args: [freelancerAddr, finalEvaluator, tokenAddr, amountInWei],
      });

      if (txHash) {
        addLog(`Initiating Job creation for ${amount} ARC...`, "SYSTEM");

        const receipt = await waitForTransactionReceipt(config, {
          hash: txHash,
        });

        // Parse logs to find the Job ID
        const iface = new ethers.Interface(VANTAGE_ESCROW_ABI);
        let newJobId = null;
        ...
        for (const log of receipt.logs) {
          try {
            ...
            if (parsedLog.name === "JobCreated") {
              newJobId = parsedLog.args[0]; // The first arg is the jobId
              break;
            }
          } catch (e) {
            continue;
        ...
        if (newJobId === null) {
          throw new Error("Could not find valid Job ID in logs.");
        }

        // Update state and storage with the new ID
        setActiveJobId(newJobId);
        localStorage.setItem("vantage_active_job", newJobId.toString());

        addLog(
          `Job Created Successfully! ID: ${newJobId.toString()}`,
          "SYSTEM",
        );
  ...
  const handleReleaseFunds = async () => {
    try {
      ...
      // 1. Get the Job ID
      const targetJob = localStorage.getItem("vantage_active_job");

      if (targetJob === null) {
        addLog("Error: No active Job found to release.", "SYSTEM");
        alert("Action Error: No active job selected.");
        return;
      }

      // 2. Setup Single Contract
      const escrowContract = new ethers.Contract(
        VANTAGE_ESCROW_ADDRESS,
        VANTAGE_ESCROW_ABI,
        signer,
      );

      addLog(`Releasing funds for Job ID: ${targetJob}...`, "SYSTEM");

      // 3. Execute ERC-8183 resolveJob function
      // True = Approved (Pay Provider), False = Rejected (Refund Client)
      const tx = await escrowContract.resolveJob(targetJob, true);
      addLog("Transaction pending...", "SYSTEM");

      await tx.wait();

      addLog(`Release Confirmed! ${amount || "Funds"} sent.`, "FINISH");
      ...
      localStorage.removeItem("vantage_active_job");
      setActiveJobId(null);
      setBotStatus({ status: "IDLE" });
    } catch (err) {
      console.error("Release Error:", err);
      const msg =
        err.reason || "Execution Reverted. Ensure you are the Evaluator.";
      addLog(`Error: ${msg}`, "SYSTEM");
      alert(msg);
    }
  };

  // Alternate Ethers-based generalized function update
  const createJobDirect = async (
    freelancerAddress,
    evaluatorAddress,
    tokenAddress,
    amountInArc,
    ...
    const tx = await escrowContract.createJob(
      ...
      ethers.getAddress(evaluatorAddress),
  ...
  //         });
  //       } catch (addError) {
  //         console.error("Error adding the Arc Testnet:", addError);
  //       }
  //     } else {
  ...
  // Optional: Add an effect to check the network on load/account change
  // useEffect(() => {
  //   checkNetwork();
    ...
    <main className="min-h-screen bg-[#0A0A0B] text-slate-200 font-sans selection:bg-blue-500/30">
      ...
      <nav className="border-b border-white/5 bg-black/40 backdrop-blur-2xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 lg:px-6 h-20 flex justify-between items-center">
          ...
          {/* 3. Only render the balance if the wallet is actually connected */}
          {isConnected && (
            <div className="flex items-center mr-4 text-sm font-semibold tracking-wide text-white/80">
              Balance:
              {isLoading ? (
                // Show a sleek loading pulse while Wagmi fetches from the blockchain
                <span className="animate-pulse ml-2 text-white/50">
                  Fetching...
                </span>
              ) : (
                // Safely render the formatted number, defaulting to 0 if it fails, rounded to 4 decimals
              ...
              {/* Safely render the symbol, defaulting to USDC */}
          ...
          <div className="flex gap-4 items-center">
            {/* Red Warning Button: Shows ONLY when the user is explicitly connected 
              to a wallet, but their active chain ID does not match Arc 
            */}
            {isConnected && chainId !== ARC_TESTNET_ID && (
              <button
                onClick={handleSwitchNetwork}
                className="flex bg-red-500/10 border border-red-500/30 text-red-500 text-[10px] font-black uppercase tracking-widest px-4 py-2.5 rounded-xl hover:bg-red-500/20 transition-all items-center gap-2 shadow-[0_0_15px_rgba(239,68,68,0.2)]"
              >
            ...
            {/* The clean wallet connector profile/button */}
            <ConnectWallet />
          </div>
        </div>
      </nav>
          ...
          <div className="bg-[#111113] border border-white/5 p-6 lg:p-10 rounded-[2rem] lg:rounded-[3rem] shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row gap-4 mt-8 pt-8 border-t border-white/5 items-center">
              ...
              <button
                ...
                disabled={!freelancerAddr || isTxLoading || !isCorrectNetwork}
              ...
              {botStatus?.status === "WORK_SUBMITTED" ? (
                <div className="p-6 lg:p-8 bg-blue-500/10 border border-blue-500/20 rounded-[2rem] animate-in zoom-in duration-300">
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-800 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] transition-all shadow-xl shadow-blue-600/20"
                  >
